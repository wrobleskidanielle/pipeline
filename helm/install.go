...
func ensureDefaultRepos(env helmEnv.EnvSettings) error {
	var repos = []struct {
	...
	}{
		{
			...
			url:  viper.GetString("helm.stableRepositoryURL"),
		},
		{
			name: phelm.BanzaiRepository,
			url:  viper.GetString("helm.banzaiRepositoryURL"),
		},
		{
			name: phelm.LokiRepository,
			url:  viper.GetString("helm.lokiRepositoryURL"), // todo (colin) use const,
		},
	}
